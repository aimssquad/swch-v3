    ...
    public function saveLeaveType(Request $request)
    {
        try {
            if (!empty(Session::get("emp_email"))) {
                ...
                if ($validate->fails()) {
                    return redirect("leave-management/new-leave-type")
                        ->withErrors($validate)
                        ->withInput();
                }
    ...
    public function saveAddLeaveRule(Request $request)
    {
        try {
            if (!empty(Session::get("emp_email"))) {
                ...
                if ($validator->fails()) {
                    return redirect("leave-management/save-leave-rule")
                ...
                if (
                    ...
                    foreach (
                        ...
                        if (empty($leave_month)) {
                            ...
                            return redirect(
                                "leave-management/leave-allocation-listing"
                            );
                        }
...
// 		foreach($leave_allocations as $leave_allocationkey=>$leave_allocation){
// // 		dd($leave_allocation->leave_type_id);
// 	//->where('month_yr','=',date('m').'/'.date('Y')) 
        ...
        try {
            ...
            //     ->get();
               ...
               $leave_rs = leaveAllocation::leftJoin(
                ...
                return redirect("leave-management/leave-report");
            } else {
                return redirect("/");
            }
        } catch (Exception $e) {
    ...
    public function spoLeaveBalance(Request $request)
    {
        try {
            if (!empty(Session::get("emp_email"))) {
                ...
                return redirect("leave-management/leave-balance");
            } else {
                return redirect("/");
            }
        } catch (Exception $e) {
