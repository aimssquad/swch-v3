...
class AttendanceController extends Controller
{
    ...
    public function viewMonthlyAttendanceReport()
    {
        if (!empty(Session::get('emp_email'))) {
            $email = Session::get('emp_email');
            $Roledata = DB::table('registration')->where('status', '=', 'active')
            ...
            return view('attendance/monthly-report-list', $data);
        } else {
            return redirect('/');
        }
    ...
    public function getMonthlyReportAttandance(Request $request)
    {
        if (!empty(Session::get('emp_email'))) {
            $email = Session::get('emp_email');
 ...
 */
            ...
            for ($we = date('d', strtotime($start_date)); $we <= $total_wk_days; $we++) {
                ...
                $duty_auth = $this->getDutyRoaster($employee_code, $Roledata->reg, $new_f, $new_f);
                //dd($duty_auth);

                $offg = array();

                if (!empty($duty_auth)) {
                    $shift_auth = $this->getShiftInfo($duty_auth->shift_code, $Roledata->reg);
                    $off_auth = $this->getOffDayInfoViaShiftCode($duty_auth->shift_code, $Roledata->reg);
                }
                            ...
                            if ((!empty($laeveppnre) || !empty($laeveppnrejj)) && $join_date != $new_f && $add == 'no') {
                                ...
                                if (in_array(date('l', strtotime($new_f)), $offg)) {
                                    if (in_array($new_f, $offgholi)) {
                                        ...
                                        $month_entrynew = DB::table('attandence')
                                            ->where('month', '=', date('m/Y', strtotime($start_date)))
                                            ->where('date', '=', $new_f)
                                            ->where('employee_code', '=', $job_details->emp_code)
    ...
    public function viewattendanabsentreportN($absent_id, $year_value)
    {
        if (!empty(Session::get('emp_email'))) {
            $email = Session::get('emp_email');
            $Roledata = DB::table('registration')->where('status', '=', 'active')
            ...
            for ($y = 1; $y < 13; $y++) {
                ...
                $employee_rs = DB::Table('employee')
                    ...
                    //dd($employee_rs);
                    if (!empty($employee_rs) ){
                        $join_date = '';
                        if ($employee_rs->emp_doj != '1970-01-01') {
                            if ($employee_rs->emp_doj != '') {
                                $join_date = $employee_rs->emp_doj;
                            }
                        }
                        ...
                        for ($m = 1; $m < 32; $m++) {
                            ...
                            $start_date=$year_value . '-' . $y . '-' . '01';
                            $end_date=$year_value . '-' . $y . '-' . $last_day;
                                ...
                                if ($join_date <= $new_f) {
                                    ...
                                    if (!empty($duty_auth)) {
                                        ...
                                        if ($off_day >= 0) {
                                            ...
                                            if (in_array($new_f, $offgholi) && (empty($laevepp))) {
                                                $resultnew .= '<td style="color:#c4952c">PH</td>';
                                            } elseif ((!empty($laeveppnre) || !empty($laeveppnrejj)) && $join_date != $new_f && $add == 'no') {
                                                // dd($laeveppnre);
                                                ...
                                                //$resultnew .= '<td>' . $lc . '</td>';
                                                if ($lc == 'H') {
                                                    $resultnew .= '<td style="color:#742d5b">' . $lc . '</td>';
                                                } elseif ($lc == 'SL') {
                                                    $resultnew .= '<td style="color:#3678b8">' . $lc . '</td>';
                                                } elseif ($lc == 'A') {
                                                    $resultnew .= '<td style="color:#5bbb5c;">' . $lc . '</td>';
                                                } else {
                                                    $resultnew .= '<td>' . $lc . '</td>';
                                                }
                                                ...
                                                $resultnew .= '<td style="color:#35b0f3">P</td>';
                                            } else {
                                                //dd(in_array(date('l', strtotime($new_f)), $offg));
                                                if (in_array(date('l', strtotime($new_f)), $offg)) {
                                                    // dd('in false');
                                                    if (in_array($new_f, $offgholi)) {
                                                        ...
                                                        $month_entrynew = DB::table('attandence')->where('month', '=', date('m/Y', strtotime($start_date)))->where('date', '=', $new_f)->where('employee_code', '=', $employee_rs->emp_code)->where('emid', '=', $data['Roledata']->reg)->get();
                                                        ...
                                                        $resultnew .= '<td style="color:#a29b9b">Off</td>';
                            ...
                            if ($last_day >= $m) {
                                ...
                                if ($join_date <= $year_value . '-' . $y . '-' . $m) {
                                    ...
                                    if (in_array($year_value . '-' . $y . '-' . $m, $offgholi) && (empty($laevepp))) {
                                        ...
                                        if (!empty($laeveppnrejj) && $join_date != $nfd) {
                                            $laeveppnrnamee = DB::table('leave_type')
                                        ...
                                        if (empty($duty_auth_new_oow)) {
                                            ...
                                            if (empty($Roledatad)) {
                                                ...
                                                $resultnew .= '<td style="color:#5bbb5c;">A</td>';
                            ...
                            if ($last_day >= $m) {
                               ...
                               // dd($Roledatad);
                                ...
                                $laevepp = DB::table('leave_apply')
                                    ...
                                    ->where('employee_id', '=', $emp->emp_code)
                                    ->where('emid', '=', $Roledata->reg)
                                    ->where('from_date', '<=', $new_f_sm)
                                    ->where('to_date', '>=', $new_f_sm)
                                    ->where('status', '=', 'APPROVED')
                                    ->orderBy('id', 'DESC')
                                    ->first();
                                $laeveppnrejj = DB::table('leave_apply')
                                ...
                                if ($join_date <= $new_f_sm) {
                                    ...
                                    if (in_array($new_f_sm, $offgholi) && (empty($laevepp))) {
                                        $resultnew .= '<td style="color:#c4952c">PH</td>';
                                    } else if (!empty($att) && empty($laevepp) && empty($laeveppnrejj)) {

                                        $resultnew .= '<td style="color:#35b0f3">P</td>';
                                    } else if ((!empty($laevepp) || !empty($laeveppnrejj)) && $join_date != $new_f_sm && $add == 'no') {
    ...
    public function viewattendanabsentreportpdfN($absent_id, $year_value)
    {
        if (!empty(Session::get('emp_email'))) {
            $email = Session::get('emp_email');
            $Roledata = DB::table('registration')->where('status', '=', 'active')
