...
<style>
  ...
  <body>
    <div class="form-body-admin" style="background: url({{asset('public/img/login-bg-ec.webp')}}) no-repeat center center;">
		<div class="login-form">
						<div class="col-md-12">
						  <h3><img  style="width: 60px;" src="{{ asset('img/logo.png')}}"> WorkPermitCloud</h3>
						  <h4 style="font-size:18px;margin-top: 30px;color: #388de4;font-weight: 400;border-bottom: 1px solid #999;padding-bottom: 10px;"> User Login</h4>
						  <h4 style="font-size:15px;">Select Organisation For Proceed</h4></div>
					<form action="{{url('user-check-organisation')}}"  method="post" id="my_captcha_form">
						{{csrf_field()}}
			...
			@endif
							...
							<div class="col-md-12">
							    <select  class="form-control select2_el" required name="com" id="com">
 ...
 <?php
$ff = DB::table('registration')->where('reg', '=', $dept->module_name)->where('status', '=', 'active')->first();
?>
 @if(!empty($ff))
                     <option value="{{$dept->module_name}}" >{{$ff->com_name}}</option>
				 @endif
                       @endforeach
                        </select>
