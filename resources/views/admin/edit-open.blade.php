						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header custom-card">
									<h4 class="card-title"><i class="far fa-thumbs-down"></i> Open Complain</h4>
									@if(Session::has('message'))										
							<div class="alert alert-success" style="text-align:center;"><span class="glyphicon glyphicon-ok" ></span><em > {{ Session::get('message') }}</em></div>
					@endif
								</div>
								<div class="card-body">
									 <form  method="post" action="{{ url('superadmin/add-complain') }}" enctype="multipart/form-data" >
									 {{csrf_field()}}
									 <input id="newid"  type="hidden" class="form-control input-border-bottom" name="newid"  value="{{ $open->id }}" >
										<div class="row form-group">
											...
											<div class="form-group">
											    	<label for="type" class="placeholder" style="margin-top:-18px;">   Project Name</label>
							...
							<option value="Skilled Worker Route" <?php    if($open->p_name=='Skilled Worker Route'){ echo 'selected';} ?>  >Skilled Worker Route</option>
											...
											<div class="form-group">
											    	<label for="type" class="placeholder" style="margin-top:-18px;">  Complain Type</label>
                     ...
                     <option value="{{$open->cat_name}}"  selected>{{$open->cat_name}}</option>
											 ...
											 ?> >
										    <div class="form-group" >
										        		<label for="others" class="placeholder" style="margin-top:-18px;">Give Details </label>
										...
										<div class="col-md-4">
										  	<div class=" form-group">		
										  <label for="emid" class="placeholder" style="margin-top:-18px;"> select Organisation </label>
...
$ff= DB::table('registration')->where('reg','=',$dept->module_name)->first();
 ?>
                     <option value="{{$dept->module_name}}" <?php    if($open->emid==$dept->module_name){ echo 'selected';} ?>>{{$ff->com_name}}</option>
                       @endforeach	<label for="emid" class="placeholder" style="margin-top:-18px;"> select Organisation </label>
											...
											<div class="form-group">
											    	<label for="descrption" class="placeholder" style="margin-top:-18px;">Description</label>
										 ...
										 <div class="col-md-6">
											...
											<div class="form-group">
												...
												<label for="flie" class="placeholder"> Uplaod File </label>
												  <p></p><a href="{{asset($open->flie)}}" download><img  style="width: 25px;" src="{{ asset('img/upload.png')}}"></a></p>
											</div>
