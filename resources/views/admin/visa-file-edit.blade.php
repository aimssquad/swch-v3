						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="fas fa-file-word"></i> Edit Visa  File</h4>
								</div>
								<div class="card-body">
									<form action="{{url('superadmin/edit-visa-file')}}" method="post" enctype="multipart/form-data" id="myForm">
			 							{{csrf_field()}}

			 							<input id="id" type="hidden"  name="id"    class="form-control input-border-bottom"  style="margin-top: 22px;" value="{{$hr->id}}" >
										 <input id="in_cos" type="hidden"  name="in_cos">
										<div class="row form-group">
											...
											<div class="col-md-4">
												<div class="form-group">
													...
													@php
														//echo $hr->employee_name;

														if($hr->master_applicant_id >0){
															$candidate = DB::table('visa_file_emp')
																		->where('emid', '=', $res->reg)
																		->orderBy('employee_name', 'asc')
																		->get();
														}else{
															$candidate = DB::table('cos_apply_emp')
																		->where('emid', '=', $res->reg)
																		->orderBy('employee_name', 'asc')
																		->get();
														}
													...
													<select class="form-control input-border-bottom" id="employee_name"   name="employee_name" disabled>
														...
														@foreach($candidate as $record)
															<option value="{{$record->employee_name}}" @if(isset($hr->employee_name) && $hr->employee_name==$record->employee_name) selected @endif>{{$record->employee_name}}</option>
														@endforeach
													</select>
													@endif
