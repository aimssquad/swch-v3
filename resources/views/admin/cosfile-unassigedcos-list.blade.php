...
<body>
  ...
  @include('admin.include.header')
	...
	<h4 class="card-title"><i class="fas fa-list"></i> COS Unassigned List @if(count($cos_granted_unassigned_rs)!=0)

	<form  method="post" action="{{ url('superadmin/list-unsignedcos-cosfile-export') }}" enctype="multipart/form-data" >
					...
					@endif
								...
								<div class="card-body">
									<form  method="get" action="{{ url('superadmin/list-unsignedcos-cosfile') }}" enctype="multipart/form-data" >
                                    	<input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    	<div class="row form-group">
                                            <div class="col-md-5">
								     		...
								     		<div class="col-md-2 btn-up">
											  ...
											  <a class="btn btn-primary" href="{{ url('superadmin/list-unsignedcos-cosfile') }}">Reset</a>
								     		</div>
										</div>
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id="basic-datatables" class="display table table-striped table-hover" >
											...
											<tbody>
											  <?php $i = 1;?>
						@foreach($cos_granted_unassigned_rs as $company)
								<?php

if (!empty($company->target_date)) {

    $sa_date = date('d/m/Y', strtotime($company->target_date));
...
if (!empty($company->visa_date)) {

    $app_date = date('d/m/Y', strtotime($company->visa_date));
...
?>
				<tr>
                            ...
                            <td>
                                @if($company->applied_cos_date != NULL && $company->applied_cos_date != '0000-00-00')
                                {{ date('d/m/Y',strtotime($company->applied_cos_date)) }}
                                @endif
                            </td>

                            <td>
                                @if($company->additional_info_request_date != NULL && $company->additional_info_request_date != '0000-00-00')
                                {{ date('d/m/Y',strtotime($company->additional_info_request_date)) }}
                                @endif
                            </td>
                            <td>
                                @if($company->additional_info_sent_date != NULL && $company->additional_info_sent_date != '0000-00-00')
                                {{ date('d/m/Y',strtotime($company->additional_info_sent_date)) }}
                                @endif
                            </td>

                            <td>{{ ($company->status == '') ? 'Pending' : $company->status }}</td>
                            <td>
                                @if($company->cos_assigned_date != NULL && $company->cos_assigned_date != '0000-00-00')
                                {{ date('d/m/Y',strtotime($company->cos_assigned_date)) }}
                                @endif
                            </td>
                            <td>{{ $company->remarks_cos }}</td>
							<td>
