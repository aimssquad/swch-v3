			...
			<div class="content">
				<div class="page-inner">
						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="far fa-building"></i> Organisation Assigned For Recruitment @if(count($companies_rs)!=0)
									<form  method="post" action="{{ url('superadmin/list-assigned-recruitmentfile-export') }}" enctype="multipart/form-data" >
					...
					@endif
								...
								<div class="card-body">
									<form  method="get" action="{{ url('superadmin/list-assigned-recruitmentfile') }}" enctype="multipart/form-data" >
                                    	<input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    	<div class="row form-group">
                                            <div class="col-md-5">
								     		...
								     		<div class="col-md-2 btn-up">
											  ...
											  <a class="btn btn-primary" href="{{ url('superadmin/list-assigned-recruitmentfile') }}">Reset</a>
								     		</div>
										</div>
										...
										<table id="basic-datatables" class="display table table-striped table-hover" >
											<thead>
												<tr>
													...
													<th>Phone No.</th>
													<th>Org. Status</th>
													<th>Org. Verification</th>
													<th>Org. License Applied</th>
													<th>Org. Type</th>
													<th>Org. Registered On</th>
													<th>SWC Employee</th>
													<th>Assigned On</th>
												</tr>
											</thead>
						...
						<tr>
							...
							<td>{{ strtoupper($company->status) }}</td>
							    <td>
								   ...
								   <td>@if($company->created_at!='')  {{ date('d-m-Y',strtotime($company->created_at))  }}@endif</td>
								   <td>{{$company->caseworker}}</td>
								   <td>@if($company->assign_date!='')  {{ date('d-m-Y',strtotime($company->assign_date))  }}@endif</td>
