			...
			<div class="content">
				<div class="page-inner">
						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="far fa-building"></i> Billed 2<sup>nd</sup> Organisation @if(count($companies_rs)!=0)

									<form  method="post" action="{{ url('superadmin/billed-second-invoice-internal-export') }}" enctype="multipart/form-data" >
					...
					@endif
								...
								<div class="card-body">
									<form  method="get" action="{{ url('superadmin/billed-second-invoice-internal') }}" enctype="multipart/form-data" >
                                    	<input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    	<div class="row form-group">
                                            <div class="col-md-5">
								     		...
								     		<div class="col-md-2 btn-up">
											  ...
											  <a class="btn btn-primary" href="{{ url('superadmin/billed-second-invoice-internal') }}">Reset</a>
								     		</div>
										</div>
...
?>
						<tr>
							...
							<td>@if($company->address!=''){{ $company->address }} @if($company->address2!=''),{{ $company->address2 }}@endif,{{  $company->road }},{{  $company->city }},{{  $company->zip }},{{  $company->country }}@endif {{ $company->email }}</td>
							...
							<!-- <td>{{ strtoupper($company->status) }}</td>
							    <td>
								   ...
								   <td>@if($company->bill_date!='')  {{ date('d-m-Y',strtotime($company->bill_date))  }}@endif</td>
								   <td>{{ $company->invoice_amount }}</td>
								   <td>{{ $company->invoice_no }}</td>
							     <td>{{ $company->invoice_due }}</td>
							     <td>{{ \Helpers::getLastBillRemarks($company->invoice_no) }}</td>
							       <!-- <td class="drp">
