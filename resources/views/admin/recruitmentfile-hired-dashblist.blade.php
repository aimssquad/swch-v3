...
<body>
  ...
  @include('admin.include.header')
	...
	<h4 class="card-title"><i class="fas fa-list"></i> Recruitment File Hired List @if(count($recruitementfile_hired_rs)!=0)

	<form  method="post" action="{{ url('superadmin/list-hired-recruitment-file-export') }}" enctype="multipart/form-data" >
					...
					@endif
								...
								<div class="card-body">
									<form  method="get" action="{{ url('superadmin/list-hired-recruitment-file') }}" enctype="multipart/form-data" >
                                    	<input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    	<div class="row form-group">
                                            <div class="col-md-5">
								     		...
								     		<div class="col-md-2 btn-up">
											  ...
											  <a class="btn btn-primary" href="{{ url('superadmin/list-hired-recruitment-file') }}">Reset</a>
								     		</div>
										</div>
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id="basic-datatables" class="display table table-striped table-hover" >
											...
											<tbody>
											<?php $i = 1;?>
						@foreach($recruitementfile_hired_rs as $company)
				<tr>
							...
							<td>{{ $company->employee_name }}</td>
                               ...
                               <td>{{ $company->advert_posted }}</td>
                                <td>{{$company->advert_start_date}}</td>
                                <td>{{$company->advert_end_date}}</td>

							<td>
{{$company->date_of_interview}}
                  </td>
                  <td>
{{$company->candidate_hired}}
                  </td><td>
{{$company->candidate_hired_date}}
                  </td>
							     <td>
                 {{$company->remarks}}
                  </td>
