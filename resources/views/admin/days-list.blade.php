  ...
  @include('admin.include.header')
		...
		<div class="main-panel">
			<div class="page-header">
						<!-- <h4 class="page-title"> 30 Days+ List</h4> -->
						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="fas fa-list"></i> 30 Days+ List </h4>
   ...
   ->orderBy('billing.id', 'desc')
    ->select('billing.*','tareq_app.ref_id')
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id="basic-datatables" class="display table table-striped table-hover" >
											...
											<tbody>
											  <?php $i = 1; ?>
							@foreach($bill_rs as $companyhh)
								<?php
								
								 if($companyhh->status=='not paid' && $companyhh->hold_st!='Yes' || $companyhh->hold_st!=' ' || is_null($companyhh->hold_st)) {
								 
								     	
				        $daten=date('Y-m-d',strtotime($companyhh->date.'  + 30 days'));
				        if($daten<date('Y-m-d')){
						
								$pass=DB::Table('registration')
		        
				 ->where('reg','=',$companyhh->emid) 
				...
				->first();
				  ...
				  ->where('employee_id','=',$companyhh->ref_id) 
				...
				<tr>
							...
							<td>{{ $i }}</td>
							<td>{{ $companyhh->in_id }}</td>
							<td>{{ $pass->com_name }}</td>
                                                                           
							<td>{{$compankky }}</td>
						
                               <td>{{ $companyhh->amount }}</td>
                              <td>{{ date('d/m/Y',strtotime($companyhh->date)) }}</td>
                              
							     
                           
							<td>{{ strtoupper($companyhh->status) }}</td>
