...
<body>
  ...
  @if( $offer->emp_pr_pincode) ,{{ $offer->emp_pr_pincode}} @endif  @if( $offer->emp_pr_country) ,{{ $offer->emp_pr_country}} @endif</h3></td>
		...
		<tr>
			<td colspan="2">Date : {{date('d/m/Y',strtotime($offer->euss_exp_date.'  - 60  days'))}}</h3></td>
			
		</tr>

		<tr>
			<tr><td colspan="2">
				<p><b>Subject: Right to Work Documentation – Temporary EUSS 60-day Reminder.</b>
<p>Further to your employment on a temporary EUSS, I am writing to remind you that this EUSS is due to
expire on {{date('d/m/Y',strtotime($offer->euss_exp_date))}}. You are therefore requested to make arrangements to renew your right to
...
<p>{{ $Roledata->desig }}</p>
			...
			<tr><td colspan="2"><input style="background: #2196f3;
    color: #fff;box-shadow: none;" type="button" value="Print this page" onClick="window.print()"></td></tr>
	</table>
