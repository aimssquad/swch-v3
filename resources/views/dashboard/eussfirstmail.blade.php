...
<body>
  ...
  @if( $offer->emp_pr_pincode) ,{{ $offer->emp_pr_pincode}} @endif  @if( $offer->emp_pr_country) ,{{ $offer->emp_pr_country}} @endif</h3></td>
		...
		<tr>
			<td colspan="2">Date : {{date('d/m/Y',strtotime($offer->euss_exp_date.'  - 90  days'))}}</h3></td>
			
		</tr>

		<tr>
			<tr><td colspan="2">
				<p><b>Subject: Right to Work Documentation – Temporary EUSS 90-day Reminder.</b>
...
<p>{{ $Roledata->desig }}</p>
			...
			<tr><td colspan="2"><input type="button" value="Print this page" onClick="window.print()"></td></tr>
		</tr>
	</table>
