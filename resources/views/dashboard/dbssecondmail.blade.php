...
<body>
  ...
  @if( $offer->emp_pr_pincode) ,{{ $offer->emp_pr_pincode}} @endif  @if( $offer->emp_pr_country) ,{{ $offer->emp_pr_country}} @endif</h3></td>
		...
		<tr>
			<td colspan="2">Date : {{date('d/m/Y',strtotime($offer->pass_exp_date.'  - 60  days'))}}</h3></td>
			
		</tr>

		<tr>
			<tr><td colspan="2">
				<p><b>Subject: Right to Work Documentation – Temporary DBS 60-day Reminder.</b>
<p>Further to your employment on a temporary DBS, I am writing to remind you that this DBS is due to
expire on {{date('d/m/Y',strtotime($offer->dbs_exp_date))}}. You are therefore requested to make arrangements to renew your right to
