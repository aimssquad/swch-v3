...
<body>
						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="fa fa-hourglass-end" aria-hidden="true" style="color:#10277f;"></i>&nbsp;Grace Period Details</h4>
								</div>
								<div class="card-body">
									<form action="{{ url('approta/add-grace-period') }}" method="post" enctype="multipart/form-data">
			 {{csrf_field()}}
	...
	<label for="shift_code" class="placeholder">Shift Code</label>
										     <select  id="shift_code"  name="shift_code"class="form-control input-border-bottom" required=""  style="" onchange="chngdepartmentshiftcode(this.value);">
											    <option value="">&nbsp;</option>
                     ...
                     <option value="{{$shiftcval->id}}" <?php  if(app('request')->input('id')){  if($shift_management->shift_code==$shiftcval->id){ echo 'selected';} } ?>>{{$shiftcval->shift_code}} ( {{ $shiftcval->shift_des }}  )</option>
                       ...
                       @endif
										...
										<div class="col-md-3">
										  	<div class=" form-group">	
<label for="time_in" class="placeholder">Work In Time</label>

										     <input id="time_in"  name="time_in" value="<?php  if(app('request')->input('id')){ echo $shift_management->time_in; } ?>" type="time" class="form-control input-border-bottom" required="" readonly  placeholder="">
										...
										<div class="row form-group">
										   ...
										   <div class="col-md-3">
										    <div class=" form-group">	
<label for="inputFloatingLabel-shift-in-time" class="placeholder">Grace Period</label>

										     <input id="inputFloatingLabel-shift-in-time" type="time" class="form-control input-border-bottom"  required="" name="grace_time" value="<?php  if(app('request')->input('id')){ echo $shift_management->grace_time; } ?>"   placeholder="">
...
$.ajax({
				type:'GET',
				url:'{{url('approta/get-employee-all-details-shift')}}/'+val+'/'+emid,
				success: function(response){
