...
<div class="main-dash">
    ...
    text-align: center;
    margin-bottom: 0;
    padding: 5px 5px 8px;" class="text fw-bold">List of Employees</h3>
		</div>
		<div class="col-md-12">
				<div class="accordion" id="accordionExample">
<?php $j=1;?>
			@if(count($dailyattandence))
				 
				@foreach( $dailyattandence  as $value)
		...
		<a class="btn btn-link @if($j!='1') collapsed @endif" data-toggle="collapse" data-target="#collapse<?=$j?>">
			<div class="card-header">
					...
					<i class="fa fa-plus"></i>   {{$value['emp_fname']}}   {{$value['emp_mname']}}   {{$value['emp_lname']}}  (Code :{{$value['employee_code']}} )
			...
			<div class="collapse @if($j=='1') show @endif" id="collapse<?=$j?>" data-parent="#accordionExample">
				<div class="card-body">
@if(count($value['log'])!=0)
											@foreach($value['log'] as $logvalue)
										<?php  $time_in='';
			 if($logvalue['timein']!=''){
				 
				$time_in=date('h:i a',strtotime($logvalue['timein'])); 
			 }
			 $time_out='';
			 if($logvalue['timeout']!=''){
				 
				$time_out=date('h:i a',strtotime($logvalue['timeout'])); 
			 }
			 ?>
<div class="table-responsive">
										<table class="table">
										<tr><td><b>Clock-In :</b></td> <td>{{$time_in}}</td></tr>
										
										<tr><td><b>Clock-In  Location:</b></td> <td> {{$logvalue['time_in_location']}}</p></td></tr>
										<tr><td><b>Clock-Out :</b></td> <td> {{$time_out}}</p></td></tr>
										<tr><td><b>Clock-Out  Location:</b></td> <td> {{$logvalue['time_out_location']}}</p></td></tr>
										<tr><td><b>Duty Hours:</b></td> <td> {{$logvalue['duty_hours']}}</p></td></tr>
                                      </tr>
</table>
</div>
										 @endforeach
