...
<body>
						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="fa fa-refresh" aria-hidden="true" style="color:#10277f;"></i>&nbsp;Late Policy Details</h4>
								</div>
								<div class="card-body">
									<form action="{{ url('approta/add-late-policy') }}" method="post" enctype="multipart/form-data">
			 {{csrf_field()}}
	...
	<label for="shift_code" class="placeholder">Shift Code</label>
										     <select  id="shift_code"  name="shift_code"class="form-control input-border-bottom" required=""  style="">
											    <option value="">&nbsp;</option>
                     ...
                     <option value="{{$shiftcval->id}}" <?php  if(app('request')->input('id')){  if($shift_management->shift_code==$shiftcval->id){ echo 'selected';} } ?>>{{$shiftcval->shift_code}} ( {{ $shiftcval->shift_des }}  )</option>
                       ...
                       @endif
										 ...
										 <div class="col-md-3">
										  	<div class=" form-group">	
<label for="inputFloatingLabel-shift-description" class="placeholder">Maximum Grace Period in Minutes</label>
										...
										<div class="row form-group">
										   ...
										   <div class="col-md-3">
										    <div class=" form-group">	
	<label for="inputFloatingLabel-shift-in-time" class="placeholder">No. of Days Allow</label>
										     <input id="inputFloatingLabel-shift-in-time" type="number" class="form-control input-border-bottom"  required="" name="no_allow" value="<?php  if(app('request')->input('id')){ echo $shift_management->no_allow; } ?>"  placeholder="">
										  ...
										  <div class="col-md-3">
										  	<div class=" form-group">	
	<label for="inputFloatingLabel-shift-out-time" class="placeholder">No. of Day Salary Deducted</label>
										     <input id="inputFloatingLabel-shift-out-time"  name="no_day_red" value="<?php  if(app('request')->input('id')){ echo $shift_management->no_day_red; } ?>"  type="number" class="form-control input-border-bottom" required=""  placeholder="">
   ...
   function chngdepartment(empid){
      ...
      function chngdepartmentshift(empid){
	   var emid= "<?=$Roledata->reg;?>";
	   	$.ajax({
		type:'GET',
		url:'{{url('approta/getEmployeedesigBylateId')}}/'+empid+'/'+emid,
        cache: false,
		success: function(response){
