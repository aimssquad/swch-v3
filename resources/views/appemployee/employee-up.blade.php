						...
						<div class="col-md-12">
							<div class="card custom-card">
								<div class="card-header">
									<h4 class="card-title"><i class="far fa-user"></i> Employee Report</h4>
						 ...
						 @if(Session::has('message'))										
								...
								<div class="card-body">
								 <form  method="post" action="{{ url('appdocument/employee-report') }}" enctype="multipart/form-data" >
									...
									<select id="inputFloatingLabel-choose-year" name="em_code" class="form-control input-border-bottom" required="" onchange="checkemp(this.value);">
												...
												<option value="{{ $employee->emp_code}}" @if(isset($emp_code) &&($emp_code)==$employee->emp_code) selected @endif> {{ $employee->emp_fname}}
...
<?php  }?>
	...
	function checkemp(empid){
	    var emid= "<?=$Roledata->reg;?>";
	   	$.ajax({
		type:'GET',
		url:'{{url('appdocument/getEmployeedreportfileById')}}/'+empid+'/'+emid,
        cache: false,
